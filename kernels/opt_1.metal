...
kernel void matmul_opt_1(device const float * A [[buffer(0)]],
                ...
                uint2 local_thread_idx [[ thread_position_in_threadgroup ]])
{
    ...
    const uint j = block_x * params.BLOCK_SIZE_X + thread_x; // col
    const uint i = block_y * params.BLOCK_SIZE_Y + thread_y; // row 
